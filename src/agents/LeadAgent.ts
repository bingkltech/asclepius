...
Respond with ONLY a JSON array. No markdown fences. Each element:
{
  ...
  static autoAssign(tasks: PipelineTask[], agents: AgentConfig[]): PipelineTask[] {
    ...
    // Pre-calculate current task loads to avoid O(N^2) inner loop filter
  ...
  static tick(plan: ExecutionPlan, agents: AgentConfig[]): ExecutionPlan {
    // Pre-calculate task statuses for O(1) dependency lookup
    const taskStatusMap = new Map(plan.tasks.map(t => [t.id, t.status]));
    ...
    for (const task of plan.tasks) {
      if (task.status === 'blocked') {
        ...
        if (allDepsMet) {
          ...
          // Update map to keep state consistent if another task depends on this one in the same tick
          taskStatusMap.set(task.id, 'pending');
        }
      }
    }
