...
## 2026-04-30 - Optimize DAG Task worker lookups
**Learning:** In React components rendering execution DAG tasks, calling .find() on an array for every task creates an O(N*M) bottleneck.
**Action:** Use useMemo to pre-calculate a Map for O(1) lookups during task mapping.
